...
Snowflake. This connector supports Snowflake's functionalities listed in the table below, allowing for
...
To use the Snowflake connector, follow these steps in a Hasura project:
...
If you look at the `configuration.json` for your connector, you'll see metadata describing your Snowflake mappings.
...
MY_SUBGRAPH_MY_SNOWFLAKE_READ_URL=http://local.hasura.dev:8081
MY_SUBGRAPH_MY_SNOWFLAKE_WRITE_URL=http://local.hasura.dev:8081
```
...
This starts our Snowflake connector on the specified port. We can navigate to the following address, with the port
modified, to see the schema of our Snowflake data source:
...
include:
  - path: my_subgraph/connector/my_snowflake/docker-compose.my_snowflake.yaml
```
...
Finally, now that our `DataConnectorLink` has the correct environment variables configured for the Snowflake connector,
...
After this command runs, you can open your `my_subgraph/metadata/my_snowflake.hml` file and see your metadata completely
...
The Hasura Snowflake connector is available under the [Apache License
